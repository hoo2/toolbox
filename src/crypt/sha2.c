...
static void sha2_process (sha2_t* ctx, const uint8_t data[64]);
static void sha2_update (sha2_t *ctx, const uint8_t *input, size_t ilen);
static void sha2_finish (sha2_t *ctx, uint8_t output[32]);
static void sha2 (uint8_t *in, size_t ilen, uint8_t out[32], sha2_size sz);
 ...
 */
static void sha2_finish (sha2_t *ctx, uint8_t out[32])
{
   uint32_t last, padn;
   uint32_t high, low;
   uint8_t  msglen[8];
...
/*!
 * \brief
 *    Calculate the SHA2 hash SHA-224 digest of an input buffer
 *
 * \param input   buffer holding the  data
 * \param ilen    length of the input data
 * \param output  SHA-224 checksum result
 * \return        none
 */
void sha224 (uint8_t *input, size_t ilen, uint8_t output[28]) {
   sha2 (input, ilen, output, SHA2_224);
   /*
    * Forward call to sha2()
    */
}

/*!
 * \brief
 *    Calculate the SHA2 hash SHA-256 digest of an input buffer
 *
 * \param input   buffer holding the  data
 * \param ilen    length of the input data
...
void sha256 (uint8_t *input, size_t ilen, uint8_t output[32]) {
   sha2 (input, ilen, output, SHA2_256);
   /*
    * Forward call to sha2()
    */
}
