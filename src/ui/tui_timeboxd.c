...
__Os__ static void _mk_frame (tuid_t *tuid, time_t t, uint8_t frm)
{
   ...
   if (frm & UI_TIME_HH) {
      i += sprintf ((char*)&tuid->frame_buffer.fb[_LINE(1)+i], "%02d",  s->tm_hour);
      if (frm & UI_TIME_MM) tuid->frame_buffer.fb[_LINE(1)+i++] = ':';
   }
   if (frm & UI_TIME_MM) {
      i += sprintf ((char*)&tuid->frame_buffer.fb[_LINE(1)+i], "%02d",  s->tm_min);
      if (frm & UI_TIME_SS) tuid->frame_buffer.fb[_LINE(1)+i++] = ':';
 ...
 * \return none
 */
__Os__ static void _mk_line (tuid_t *tuid, time_t t, uint8_t frm)
{
   int i=0;
   struct tm *s;
   ...
   // Print value
   s = sgmtime(&t);
   if (frm & UI_TIME_DD)   i += sprintf ((char*)&tuid->frame_buffer.fb[i], "%dd ",   s->tm_yday);
   if (frm & UI_TIME_HH) {
      i += sprintf ((char*)&tuid->frame_buffer.fb[i], "%02d",  s->tm_hour);
      if (frm & UI_TIME_MM) tuid->frame_buffer.fb[i++] = ':';
   }
   if (frm & UI_TIME_MM) {
      i += sprintf ((char*)&tuid->frame_buffer.fb[i], "%02d",  s->tm_min);
      if (frm & UI_TIME_SS) tuid->frame_buffer.fb[i++] = ':';
      //else                  tuid->frame_buffer.fb[i++] = '\'';
   }
   if (frm & UI_TIME_SS)   i += sprintf ((char*)&tuid->frame_buffer.fb[i], "%02d", s->tm_sec);

   // discard null termination inside frame buffer
   tuid->frame_buffer.fb[i] = ' ';
   // Keep null termination at end of each line
 ...
 * ESC      --       "        "        "
 */
__Os__ ui_return_t tui_line_timeboxd (tuid_t *tuid, int live, int key, uint8_t frm, time_t up, time_t down, time_t step, time_t *value)
{
   static time_t cur, t;
   static int ev=1, speedy=0;
